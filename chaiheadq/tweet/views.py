...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save(commit=False) #To not save form right now, we put that commit=False
            user.set_password(form.cleaned_data['password1'])
            user.save()
            login(request, user)
            return redirect('tweet_list')  # Redirect to the login page after registration
    else:
